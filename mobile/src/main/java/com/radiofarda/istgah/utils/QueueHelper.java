...
import com.radiofarda.istgah.VoiceSearchParams;
...
public class QueueHelper {
    ...
    public static List<MediaSessionCompat.QueueItem> getPlayingQueue(String mediaId,
        ...
        LogHelper.d(TAG, "Creating playing queue for ", mediaId);

        List<MediaMetadataCompat> tracks = new ArrayList<>();
        // This sample only supports genre and by_search category types.
        tracks.add(musicProvider.getMusic(mediaId));
    ...
    public static List<MediaSessionCompat.QueueItem> getPlayingQueueFromSearch(String query,
        ...
        if (params.isAny) {
            ...
            // for radiofarda, favorite playlists, "I'm feeling lucky", most recent, etc.
...
//            result = musicProvider.getMusicsByGenre(params.genre);
        ...
        // to Google, for radiofarda, but is not. For radiofarda, a user searching for Madonna on
        ...
        if (params.isUnstructured || result == null || !result.iterator().hasNext()) {
            // To keep it simple for this radiofarda, we do unstructured searches on the
